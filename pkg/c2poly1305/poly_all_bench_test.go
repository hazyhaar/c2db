...
import (
	...
	"github.com/hazyhaar/c2db/pkg/c2poly1305x2"
	"github.com/hazyhaar/c2db/pkg/c2poly1305x8"
	"golang.org/x/crypto/poly1305"
)

// BenchmarkPoly1305_AllImplementations_64KiB
func BenchmarkPoly1305_AllImplementations_64KiB(b *testing.B) {
	key := make([]byte, 32)
	rand.Read(key)
	...
	b.Run("1_x_crypto_official_ASM", func(b *testing.B) {
	...
	b.Run("2_c2fused_poly_update_uint128_64bit", func(b *testing.B) {
		b.SetBytes(int64(len(payload)))
		b.ReportAllocs()
		var st c2fused.C2fused_poly
		var tag [16]byte
		for i := 0; i < b.N; i++ {
			c2fused.C2fused_poly_init(&st, key)
			c2fused.C2fused_poly_update(&st, payload, uint64(len(payload)))
			c2fused.C2fused_poly_final(&st, tag[:])
		}
	})

	b.Run("3_c2poly1305_1way_donna26", func(b *testing.B) {
		b.SetBytes(int64(len(payload)))
		b.ReportAllocs()
	...
	b.Run("4_c2poly1305x2_2way_donna26", func(b *testing.B) {
		b.SetBytes(int64(len(payload)))
		b.ReportAllocs()
	...
	b.Run("5_c2poly1305x8_8way_superposed_donna26", func(b *testing.B) {
		b.SetBytes(int64(len(payload)))
		b.ReportAllocs()
