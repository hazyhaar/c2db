#include "c2db_crc32c_fullpage.h"

static const uint32_t c2db_crc32c_table256[256] = {
...
uint32_t c2db_crc32c_fullpage(const uint8_t *page, uint64_t n) {
    ...
    i = 0;
    while (i < n) {
        if (i >= (uint64_t)C2DB_CRC_OFFSET && i < (uint64_t)(C2DB_CRC_OFFSET + 4)) {
            b = 0;
        } else {
            b = page[i];
        }
        crc = (crc >> 8) ^ c2db_crc32c_table256[(crc ^ (uint32_t)b) & 0xFF];
        i = i + 1;
    }
    return ~crc;
}
...
uint8_t c2db_crc32c_fullpage_store(uint8_t *page, uint64_t n) {
    ...
    crc = c2db_crc32c_fullpage(page, n);
    page[28] = (uint8_t)(crc & 0xFF);
    page[29] = (uint8_t)((crc >> 8) & 0xFF);
    page[30] = (uint8_t)((crc >> 16) & 0xFF);
    page[31] = (uint8_t)((crc >> 24) & 0xFF);
    return 1;
}
